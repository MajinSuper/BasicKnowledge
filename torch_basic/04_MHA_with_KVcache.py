...
class MHA(nn.Module):
    ...
    def forward(self, x: torch.Tensor, mask=None, use_kvcahce=False):
        ...
        K = K.view(B, seq, self.head_num, self.dk).transpose(2, 1)  # [B, head_num, seq_in, dk]
        V = V.view(B, seq, self.head_num, self.dk).transpose(2, 1)  # [B, head_num, seq_in, dk]

        if use_kvcahce:
            self.k_cache.append(K)  # [ [B, head_num, dk] ... ]
            self.v_cache.append(V)  # [ [B, head_num, dk] ... ]
            K = torch.cat(self.k_cache, dim=2)  # [B, head_num, seq, dk]
            V = torch.cat(self.v_cache, dim=2)  # [B, head_num, seq, dk]
...
if __name__ == '__main__':
    ...
    layer = MHA(hidden_dim=hidden_dim, head_num=4)
